...
An input is read through its own strides, so a transposed, reversed or stepped view goes to the kernel as it is, and only a view built on an index array is copied first.
