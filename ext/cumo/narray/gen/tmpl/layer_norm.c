...
/*
  ...
  @param [Cumo::<%=class_name%>] gamma  scale, as long as the last axis.
  @param [Cumo::<%=class_name%>] beta  shift, as long as the last axis.
...
<%=c_func(-1)%>(int argc, VALUE argv[], VALUE self)
{
    ...
    // Taking a pointer runs allocate, which a subclass is free to redefine and
    // reshape through, so the shapes the launch is sized from are read after.
    ...
    CUMO_CHECK_SIZE_EQ(ngamma->size, cols);
    CUMO_CHECK_SIZE_EQ(nbeta->size, cols);
