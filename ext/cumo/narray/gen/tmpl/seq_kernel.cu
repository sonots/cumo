...
void <%="cumo_#{c_iter}_#{pfx}kernel_launch"%>(<%=atype%>* a1, cumo_na_indexer_t* indexer, seq_data_t beg, seq_data_t step, seq_count_t c)
{
    ...
    case <%=idim%>:
        <%="cumo_#{c_iter}_#{pfx}kernel_dim#{idim}"%><<<grid_dim, block_dim, 0, cumo_cuda_stream()>>>(*a1,*indexer,beg,step,c);
        break;
    <% end %>
    default:
        <%="cumo_#{c_iter}_#{pfx}kernel_dim"%><<<grid_dim, block_dim, 0, cumo_cuda_stream()>>>(*a1,*indexer,beg,step,c);
        break;
    }
