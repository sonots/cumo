...
<%=c_iter%>(na_loop_t *const lp)
{
    ...
    <% else %>
    {
        if (idx2) {
            if (idx1) {
                <%="#{c_iter}_index_index_kernel_launch"%>(p1,p2,a2,idx1,idx2,i);
            } else {
                <%="#{c_iter}_stride_index_kernel_launch"%>(p1,p2,a2,s1,idx2,i);
            }
        } else {
            if (idx1) {
                <%="#{c_iter}_index_stride_kernel_launch"%>(p1,p2,a2,idx1,s2,i);
            } else {
                <%="#{c_iter}_stride_stride_kernel_launch"%>(p1,p2,a2,s1,s2,i);
            }
        }
    }
    <% end %>
