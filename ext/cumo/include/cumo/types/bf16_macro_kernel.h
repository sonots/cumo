...
__host__ __device__ static inline float cumo_bfloat_add(float x, float y) { return x + y; }
__host__ __device__ static inline float cumo_bfloat_sub(float x, float y) { return x - y; }
__host__ __device__ static inline float cumo_bfloat_mul(float x, float y) { return x * y; }
__host__ __device__ static inline float cumo_bfloat_div(float x, float y) { return x / y; }
__host__ __device__ static inline bool cumo_bfloat_not_nan(float x) { return x == x; }
__host__ __device__ static inline cumo_bfloat cumo_bfloat_add(cumo_bfloat x, cumo_bfloat y) { return cumo_float2bfloat(cumo_bfloat2float(x) + cumo_bfloat2float(y)); }
__host__ __device__ static inline cumo_bfloat cumo_bfloat_sub(cumo_bfloat x, cumo_bfloat y) { return cumo_float2bfloat(cumo_bfloat2float(x) - cumo_bfloat2float(y)); }
__host__ __device__ static inline cumo_bfloat cumo_bfloat_mul(cumo_bfloat x, cumo_bfloat y) { return cumo_float2bfloat(cumo_bfloat2float(x) * cumo_bfloat2float(y)); }
__host__ __device__ static inline cumo_bfloat cumo_bfloat_div(cumo_bfloat x, cumo_bfloat y) { return cumo_float2bfloat(cumo_bfloat2float(x) / cumo_bfloat2float(y)); }
__host__ __device__ static inline bool cumo_bfloat_not_nan(cumo_bfloat x) { return cumo_bfloat2float(x) == cumo_bfloat2float(x); }

#define m_add(x,y) cumo_bfloat_add(x,y)
#define m_sub(x,y) cumo_bfloat_sub(x,y)
#define m_mul(x,y) cumo_bfloat_mul(x,y)
#define m_div(x,y) cumo_bfloat_div(x,y)
#define m_div_check(x,y) (cumo_bfloat2float(y)==0)

__host__ __device__ static inline float cumo_bfloat_floored_mod(float x, float y)
{
    float r = fmodf(x,y);
    if (r != 0 && ((r < 0) != (y < 0))) {
        r += y;
...
#define not_nan(x) cumo_bfloat_not_nan(x)
...
__host__ __device__ static inline float cumo_bfloat_pow_positive_int(float x, unsigned int p)
{
    float r = 1.0f;
    switch (p) {
    case 0: return 1.0f;
...
__host__ __device__ static inline float cumo_bfloat_pow_int(float x, int p)
{
...
__host__ __device__ static inline cumo_bfloat cumo_bfloat_step_down(cumo_bfloat h)
{
    unsigned short b;
    cumo_bfloat r;
    memcpy(&b, &h, sizeof(b));
...
__host__ __device__ static inline float cumo_bfloat_ldexp(float x, float y)
{
    if (!(y == y)) return y;
    if (y > 64.0f) y = 64.0f;
    if (y < -64.0f) y = -64.0f;
...
__host__ __device__ static inline float cumo_bfloat_sinc(float x)
{
    return (x==0) ? 1.0f : (sinf(x)/x);
}

__host__ __device__ static inline float cumo_bfloat_sign(float x)
{
    return (x==0) ? 0.0f : ((x>0) ? 1.0f : ((x<0) ? -1.0f : x));
}

__host__ __device__ static inline cumo_bfloat f_seq(cumo_bfloat x, cumo_bfloat y, double c)
{
...
__host__ __device__ static inline float cumo_bfloat_gelu(float x)
{
    return 0.5f * x * (1.0f + erff(x * (float)CUMO_M_SQRT1_2));
}

// The approximation GPT-2 and the transformers after it were trained with.
__host__ __device__ static inline float cumo_bfloat_gelu_tanh(float x)
{
...
__host__ __device__ static inline float cumo_bfloat_silu(float x)
{
