 ...
 * cumo/types/half_macro.h and bf16_macro.h are the two includers.
 */

#define m_zero CUMO_F_TO_F16(0.0f)
#define m_one  CUMO_F_TO_F16(1.0f)

#define m_num_to_data(x) cumo_na_ruby_num_to_f16(x)
#define m_data_to_num(x) rb_float_new(CUMO_F16_TO_F(x))
...
#define m_add(x,y) CUMO_F_TO_F16(CUMO_F16_TO_F(x)+CUMO_F16_TO_F(y))
#define m_sub(x,y) CUMO_F_TO_F16(CUMO_F16_TO_F(x)-CUMO_F16_TO_F(y))
#define m_mul(x,y) CUMO_F_TO_F16(CUMO_F16_TO_F(x)*CUMO_F16_TO_F(y))
#define m_div(x,y) CUMO_F_TO_F16(CUMO_F16_TO_F(x)/CUMO_F16_TO_F(y))
#define m_div_check(x,y) (CUMO_F16_TO_F(y)==0)

static inline float cumo_f16_floored_mod(float x, float y)
{
    float r = fmodf(x,y);
    if (r != 0 && ((r < 0) != (y < 0))) {
        r += y;
...
#define not_nan(x) (CUMO_F16_TO_F(x)==CUMO_F16_TO_F(x))
...
static inline float cumo_f16_pow_positive_int(float x, unsigned int p)
{
    float r = 1.0f;
    switch (p) {
    case 0: return 1.0f;
...
static inline float cumo_f16_sinc(float x)
{
    return (x==0) ? 1.0f : (sinf(x)/x);
}

static inline float cumo_f16_sign(float x)
{
    return (x==0) ? 0.0f : ((x>0) ? 1.0f : ((x<0) ? -1.0f : x));
}

static inline dtype cumo_na_ruby_num_to_f16(VALUE x)
{
    return CUMO_D_TO_F16(NIL_P(x) ? nan("") : NUM2DBL(x));
}
...
static inline float cumo_f16_softplus(float x)
{
    float e = expf(x);
