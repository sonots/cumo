...
class BitTest < Test::Unit::TestCase
  ...
  test "a reduction over an empty Bit answers its identity" do
    ...
    # == rather than a bare assert: a zero-dimensional Bit is truthy whatever
    # it holds, so a truthiness test would pass on the wrong type.
    assert { e.all? == true }
    assert { e.any? == false }
    assert { e.none? == true }
  ...
  test "an empty Bit is told about an argument it cannot take" do
    ...
    assert_raise(Cumo::NArray::DimensionError) { e.all?(axis: 99) }
    assert_raise(Cumo::NArray::DimensionError) { e.any?(axis: 99) }
  ...
  test "the host loop count of an empty Bit is answered before the loop" do
    script = <<~'RUBY'
      require "cumo/narray"
      print [Cumo::Bit.new(0, 3).count_true_cpu,
             Cumo::Bit.new(0, 3).count_true_cpu(axis: 0)].inspect
    RUBY
    assert_equal("[0, 0]", run_child(script))
    assert_equal("[0, 0]", run_child(script, env: { "CUMO_COMPATIBLE_MODE" => "ON" }))
  end
