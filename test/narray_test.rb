...
class NArrayTest < Test::Unit::TestCase
  ...
  test "sort, sort_index, cumsum and cumprod take no keepdims:" do
    [Cumo::DFloat, Cumo::Int32].each do |dtype|
      ...
      %i[sort sort_index cumsum cumprod].each do |m|
        ...
        assert_equal(a.send(m, axis: 1).to_a, a.send(m, axis: 1, nan: true).to_a, "#{label} nan:")
      end
    end
    z = Cumo::DFloat[1, 2, 3].sum
    assert_raise(ArgumentError) { z.sort_index(keepdims: true) }
    assert_raise(ArgumentError) { z.sort(keepdims: true) }
  end
  ...
  sub_test_case "a length-one axis and an index view" do
    test "a length-one axis reads and writes the right elements" do
      ...
      assert_equal((0...12).each_slice(4).map { |r| [r.map { |x| x * 2.0 }] }, (a * 2).to_a)
      assert_equal([[6.0], [22.0], [38.0]], a.sum(axis: 2).to_a)
      assert_equal([[12.0, 15.0, 18.0, 21.0]], a.sum(axis: 0).to_a)
      b = Cumo::DFloat.new(4, 6).seq[true, 2..2]
      assert_equal([[2.0], [8.0], [14.0], [20.0]], b.to_a)
      b.fill(-1)
      assert_equal([[-1.0]] * 4, b.to_a)
      c = Cumo::Int32.new(2, 1, 3, 1).seq
      assert_equal([[[[0], [1], [2]]], [[[3], [4], [5]]]], c.to_a)
    ...
    test "rand, rand_norm, seq, real= and imag= write an index view where it is" do
      ...
      a = Cumo::SFloat.zeros(512, 256)
      rows = a[Cumo::Int32.new(128).seq(511, -4), true]
      cols = a[true, Cumo::Int32.new(64).seq(0, 4)]
      [[:rand, []], [:rand_norm, []], [:seq, []]].each do |meth, args|
        assert_equal(0, grew.call(-> { rows.send(meth, *args) }), "#{meth} rows")
        assert_equal(0, grew.call(-> { cols.send(meth, *args) }), "#{meth} cols")
      end
      a[[5, 0, 3], true].seq
  ...
  sub_test_case "a store whose source overlaps its destination" do
    ...
    test "does not trust the source's own dup" do
      n = 10_000_000
      seq = Cumo::SFloat.cast(Cumo::Int32.new(n).seq % 97)
      want = seq.dup
      want[0...n - 1024] = seq[1024..]
      a = seq.dup
      src = a[1024..]
      def src.dup = self
      a[0...n - 1024] = src
